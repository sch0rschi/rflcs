    ...
    try
    {
        ...
        for (auto match : matches)
        {
            if (match->character < constants::alphabet_size)
            {
                ...
                {
                    predecessor_sum += gurobi_variable_map[pred_match];
                }
                model.addConstr(gurobi_variable_map[match], GRB_LESS_EQUAL, predecessor_sum);
            }

            auto successor_sum = GRBLinExpr();
            for (auto& succ_match : match->dom_succ_matches)
            {
                successor_sum += gurobi_variable_map[succ_match];
            }
