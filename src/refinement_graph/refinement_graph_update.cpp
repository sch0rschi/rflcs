...
void update_refinement_graph(const instance &instance) {
    ...
    for (const auto refinement_match: instance.active_refinement_match_pointers | std::views::reverse) {
        for (const auto refinement_node_to_update_node: refinement_match->refinement_nodes) {
            ...
            globals::old_characters_on_paths_to_some_sink = refinement_node_to_update_node->characters_on_paths_to_some_sink;
            globals::old_characters_on_all_paths_to_lower_bound_levels = refinement_node_to_update_node->characters_on_all_paths_to_lower_bound_length;
            refinement_node_to_update_node->characters_on_paths_to_some_sink.reset();
            refinement_node_to_update_node->characters_on_all_paths_to_lower_bound_length.set();
            for (const auto successor_node: refinement_node_to_update_node->successors) {
                refinement_node_to_update_node->characters_on_paths_to_some_sink |=
                ...
                refinement_node_to_update_node->characters_on_all_paths_to_lower_bound_length &=
                        globals::temp_character_set_1;
                max_upperbound_down = std::max(max_upperbound_down, successor_node->upper_bound_down + 1);
            }
            refinement_node_to_update_node->upper_bound_down = max_upperbound_down;
            refinement_node_to_update_node->characters_on_paths_to_some_sink &= globals::old_characters_on_paths_to_some_sink;
            refinement_node_to_update_node->characters_on_all_paths_to_lower_bound_length |= globals::old_characters_on_all_paths_to_lower_bound_levels;
            if (refinement_match->character < instance.alphabet_size) {
                refinement_node_to_update_node->characters_on_paths_to_some_sink.reset(refinement_match->character);
            }
            if (refinement_node_to_update_node->successors.empty()
                || refinement_node_to_update_node->upper_bound_up > instance.lower_bound + 1) {
                refinement_node_to_update_node->characters_on_all_paths_to_lower_bound_length.reset();
            }

            refinement_node_to_update_node->upper_bound_down =
                    std::min(max_upperbound_down,
                             static_cast<int>(refinement_node_to_update_node->characters_on_paths_to_some_sink.
                                 count()));
            auto combined_characters = refinement_node_to_update_node->characters_on_paths_to_root
                                       | refinement_node_to_update_node->characters_on_paths_to_some_sink;
            refinement_node_to_update_node->upper_bound = std::min(
                refinement_node_to_update_node->upper_bound_up + refinement_node_to_update_node->upper_bound_down,
